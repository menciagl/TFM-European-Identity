...
# Inverse scale for "attachment to the EU": now 1 is not at all attached and 4 very attached
datos <- datos |>
  dplyr::mutate(attachment_EU = recode(as.character(attachment_EU),
                                "1" = "4",
                                "2" = "3",
                                "3" = "2",
                                "4" = "1")) |> 
  dplyr::mutate(attachment_EU = as.numeric(attachment_EU))
...
attachment_porcentaje <- datos |>
  ...
  dplyr::mutate(attachment_EU = recode(as.character(attachment_EU), !!!etiquetas_attachment)) |>
  group_by(attachment_EU) |>
  summarise(count = n()) |>
  dplyr:: mutate(porcentaje = count / sum(count) * 100) |>
  dplyr::mutate(attachment_EU = fct_relevel(attachment_EU, "Not at all", "Not very", "Fairly", "Very"))
...
# The same but with 2 categories
datos <- datos |>
  mutate(
    attachment_bin = case_when(
      attachment_EU %in% c(3, 4) ~ 1,  # Very/Fairly attached
      attachment_EU %in% c(1, 2) ~ 0,  # Not at all/Not very attached
      TRUE ~ NA_real_
    ),
    attachment_bin = factor(attachment_bin, levels = c(0, 1),
                            labels = c("Not at all / Not very", "Very / Fairly"))
  )

# Percentage of new variable
attachment_bin_porcentaje <- datos |>
  filter(!is.na(attachment_bin)) |>
  group_by(attachment_bin) |>
  summarise(count = n()) |>
  mutate(porcentaje = count / sum(count) * 100)

# Plot
ggplot(attachment_bin_porcentaje, aes(x = attachment_bin, y = porcentaje, fill = attachment_bin)) +
  geom_col(width = 0.6, show.legend = FALSE) +
  geom_text(aes(label = paste0(round(porcentaje, 1), "%")),
            vjust = -0.5, size = 4, fontface = "bold") +
...
ggsave("Resultados/Descriptive_plot_binary.jpeg", plot = last_plot(), width = 7, height = 5, dpi = 300)
...
attachment_percentages <- datos |>
  ...
  dplyr::mutate(percentage = (attached / total) * 100)
...
ggplot(data = europe_data) +
  ...
  labs(
    title = "European identity (EU) by region",
...
ggplot(evolution_data, aes(x = year, y = percentage, color = label, group = label)) +
  ...
  labs(
    title = "Evolution of European identity (EU) (2020–2023)",
    subtitle = "Distribution of responses (%)",
    x = "Year",
    y = "Percentage (%)",
...
evolution_data <- datos |>
  filter(year %in% 2020:2023, attachment_EU %in% 1:4) |>
  dplyr::mutate(response_group = case_when(
    attachment_EU %in% 1:2 ~ "No",
    attachment_EU %in% 3:4 ~ "Yes"
  )) |>
...
ggplot(evolution_data, aes(x = year, y = percentage, color = response_group, group = response_group)) +
  geom_line(linewidth = 1.2) +
  geom_point(size = 2) +
  scale_color_manual(values = c("Yes" = "#388E3C", "No" = "#E57373"), name = "Attachment") +
...
ggsave("Resultados/Descriptive_plot3_dicotomic.jpeg", plot = last_plot(), width = 7, height = 5, dpi = 300)
