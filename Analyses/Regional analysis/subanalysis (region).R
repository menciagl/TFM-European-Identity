...
importancia_df$Variable <- as.character(importancia_df$Variable)

graph1 <- ggplot(importancia_df, aes(x = reorder_within(Variable, Overall, Region), y = Overall, fill = Overall)) +
...
# Save
ggsave("Resultados/Byregion/Variable_Importance_ByRegion.jpeg", plot = graph1, width = 12, height = 8, dpi = 300)
...
ggsave("Resultados/Byregion/Logistic/logistic_combined.jpeg", plot = combined_plot, width = 20, height = 12, dpi = 300)
...
plot_pred <- plot(gg.pred1) +
  labs(
    ...
    y = "Predicted Prob. of EU Identity"
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 18, face = "bold", hjust = 0.5),
    plot.subtitle = element_text(size = 16, hjust = 0.5, color = "gray25"),
    axis.title.x = element_text(size = 17, margin = margin(t = 13)),
    axis.title.y = element_text(size = 17, margin = margin(r = 13)),
...
ggsave("Resultados/Byregion/Logistic/Predicted_Attachment_By_Bills_SocialClass.jpeg",
       plot = plot_pred, width = 8, height = 5, dpi = 300)
...
plot_center <- plot(gg.pred_center) +
  labs(
    title = "Probability of European Identity (Center)",
...
ggsave("Resultados/Byregion/Logistic/Predicted_Attachment_By_AttachmentCountry.jpeg",
       plot = plot_center, width = 8, height = 5, dpi = 300)
...
plot_north <- plot(gg.pred_north) +
  labs(
    title = "Probability of European Identity (North)",
...
ggsave("Resultados/Byregion/Logistic/Predicted_Attachment_By_EUtopics.jpeg",
       plot = plot_north, width = 8, height = 5, dpi = 300)
