...
# Plot
graph_year <- ggplot(importancia_df_year, aes(x = Variable, y = Overall)) +
...
# Save
ggsave("Resultados/Byyear/Variable_Importance_ByYear.jpeg", plot = graph_year, width = 10, height = 8, dpi = 300)
...
for (r in names(resultados_rf_cv)) {
  ...
  # PNG
  png(paste0("Resultados/Byyear/ROC/ROC_curve_", r, ".png"), width = 800, height = 600, res = 120)
  plot(roc_obj, main = plot_title, col = "black", lwd = 2)
  dev.off()
}
...
ggsave("Resultados/Byyear/Logistic/logistic_combined.jpeg", plot = combined_plot, width = 16, height = 12, dpi = 300)
